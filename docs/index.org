...

{
  ...
  services.metadata-webhook = {
    ...
    # Path to a systemd EnvironmentFile setting METADATA_WEBHOOK_SECRET (and,
    # optionally, METADATA_GITHUB_TOKEN). Keep secrets out of the Nix store.
    environmentFile = "/run/secrets/metadata-webhook";
