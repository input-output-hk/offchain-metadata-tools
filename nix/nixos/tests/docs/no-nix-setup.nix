  ...
  nodes = {
    server = { config, ... }: {
      ...
      environment.systemPackages = [
        docScripts
        metadataServerHaskellPackages.metadata-server.components.exes.metadata-server
        metadataServerHaskellPackages.metadata-webhook.components.exes.metadata-webhook
        vim
        jq
        ngrok-2
      ];
