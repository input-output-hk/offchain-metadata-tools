...
#+BEGIN_QUOTE
*To be updated.* This tutorial has not been revalidated against a
current version of =cardano-node=, and its automated test is disabled in
=nix/nixos/tests/default.nix=. It targets a retired testnet and uses
=cardano-cli= options that no longer exist. Read it as a description of
the workflow rather than as a working script.
#+END_QUOTE
...

{
  ...
  # Demo only: materialise the webhook secret as an EnvironmentFile. In
  # production supply this file from a runtime secret store (sops-nix /
  # agenix) instead of declaring it here, which places it in the Nix store.
  environment.etc."metadata-webhook.env" = {
    text = "METADATA_WEBHOOK_SECRET=${METADATA_WEBHOOK_SECRET}\n";
    mode = "0400";
  };
  ...
  services.metadata-webhook = {
    ...
    # The single GitHub repository this webhook serves; the GitHub API URL is
    # built from these, never from the (attacker-influenced) webhook payload.
    githubOwner = "cardano-foundation";
    githubRepo = "cardano-token-registry";
    environmentFile = "/etc/metadata-webhook.env";
  ...
  systemd.services.metadata-webhook.after = [ "postgresql.service" "metadata-server.service" ];
  ...
  # PostgreSQL 15+ requires explicit grants; \`ensurePermissions\` no
  # longer exists. These run in postgresql-setup.service (not
  # postgresql.service) so they fire *after* ensureDatabases has created
  # the database; that service puts \`psql\` on PATH and sets PGPORT, so a
  # bare \`psql\` connects as the superuser.
  systemd.services.postgresql-setup.postStart = pkgs.lib.mkAfter ''
    psql -tAc 'GRANT ALL PRIVILEGES ON DATABASE \${config.services.metadata-server.postgres.database} TO "\${config.services.metadata-server.postgres.user}";'
    psql -d \${config.services.metadata-server.postgres.database} -tAc 'GRANT ALL ON SCHEMA public TO "\${config.services.metadata-server.postgres.user}";'
  '';
...
#+BEGIN_QUOTE
*To be updated.* This section is written against PostgreSQL 11
configuration internals, and its automated test is disabled in
=nix/nixos/tests/default.nix=. The NixOS section above is tested and
current; prefer it where possible.
#+END_QUOTE
...
For both building with =nix build= and using =nix develop=, it might take a very long
time if you do not have the Cardano binary cache set up. Adding the
IOG binary cache to your Nix configuration will speed up builds a lot,
...
From the root directory of the project, =nix build= builds the default
package, =metadata-server=. To build a particular executable, say
=token-metadata-creator=, use =nix build .#token-metadata-creator=.

The executables provided are =metadata-server=, =metadata-sync=,
=metadata-validator-github=, =metadata-webhook= and
=token-metadata-creator=.

To build everything CI builds, including the test suites, NixOS tests
and static tarballs, use =nix build .#hydraJobs.x86_64-linux.required=.

To enter a development shell, use =nix develop=. This will enter a
development shell for which [[https://github.com/input-output-hk/haskell.nix/][haskell.nix]] provides all dependencies, and
in which =cabal build= works directly. Use =nix develop .#profiled= for
the same shell with library profiling enabled.
...
**** Preparing the release
  ...
  - [ ] Ensure that a new version of the api docs have been created
    ...
    been updated.
  - [ ] Deploy offchain-metadata-tools to the playground testnets
    (preprod/preview) such that metadata-server is available at
    https://metadata.play.dev.cardano.org/ and new metadata can be added
    in https://github.com/input-output-hk/metadata-registry-testnet.
...
**** Create the release notes
  ...
  - [ ] Add a draft release to
    ...
    - [ ] Check that the release job has attached the static tarballs and
      their sha256sums file to the draft. Pushing a =vN.N.N.N= tag runs
      it; it can be re-run from the Actions tab with workflow_dispatch.
...
**** Acceptance tests
  ...
  - [ ] Run the token-metadata-creator golden tests, which validate the
    pinned registry entries under
